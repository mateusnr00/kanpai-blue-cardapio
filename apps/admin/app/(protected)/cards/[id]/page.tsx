...
import { getCategory, listParentCandidates } from "@/lib/data/categories";
...
export default async function EditCategoryPage({ params }: { params: Params }) {
  ...
  const parents = await listParentCandidates(getActiveRestaurantId(), params.id);
  ...
  return (
    <section className="flex w-full flex-col gap-6">
      ...
      <div className="admin-card p-6 sm:p-8">
        <CategoryForm mode="edit" initial={category} parents={parents} onSubmit={onSubmit} />
      </div>
    </section>
  );
}
