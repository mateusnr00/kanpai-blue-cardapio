...
export function SlideshowImagesEditor({ initial }: Props) {
  ...
  function onAddFiles(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    for (const f of files) {
      ...
      next.push({ kind: "new", file: f, previewUrl: URL.createObjectURL(f) });
    }
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <p className="text-xs text-ink-muted">
          <ImageIcon size={14} className="inline-block align-text-bottom" weight="duotone" /> JPEG, PNG, WebP ou AVIF · máx. {MAX_SIZE_MB}MB por foto · até {MAX_FILES} fotos
        </p>
      </div>
    </div>
  );
